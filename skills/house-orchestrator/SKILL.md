...
- **Model routing — the fable-profile.** Judgment roles run on **Fable 5** (`claude-fable-5`) — the shaper
  session, the plan-check reviewer, the stage-7 merge-gate reviewer, and the panel's refuters. Throughput
  roles run on **Opus 4.8** (`claude-opus-4-8`) — this orchestrator session, every `house-builder` dispatch
  (`model: opus`), panel + health-sweep lenses, research digests, doc-reconcile. Opus builds / Fable reviews
  is deliberate: cross-ARCHITECTURE independence — a different model catches the builder-model's blind spots.
  **Escalation rung:** a unit BLOCKED twice (or a debugging stall) gets ONE re-dispatch on `model: fable`
  before you stop for the user. **Fallback:** Fable unavailable → run the opus-profile (single-Opus reviewer /
  Opus refuters) and note it in the retro.
