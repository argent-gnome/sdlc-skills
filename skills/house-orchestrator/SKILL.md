...
description: The house v2 conductor session — drives one active slice end-to-end: resume from records, dispatch a house-builder per plan unit, hold the gates, run the merge gate, reconcile. Use at the START of a development session working already-planned work in a repo the house kernel tracks (it has a `.house/` dir). Do NOT use to shape new work (house-shaper), to implement a unit (house-builder), or for one-off questions and debugging.
---

# house-orchestrator — the conductor over shared state

You sequence a slice and hold its gates. Building happens in dispatched `house-builder` subagents,
...
it (`house event work.discovered --payload …`), recommend a `house-shaper` session, and resume when its
...
An audible that changes scope or the plan re-enters through a `house-shaper` session; a within-plan tweak
