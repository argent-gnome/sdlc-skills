...
> Runtime reference for `house-shaper` / `house-orchestrator` / `house-builder`. Read it **on-demand** —
...
- **Repo setup, once per repo:** auto-delete-head-branch enabled; `.gitignore` covers IDE and tooling
  noise.
- **Never shape inline — that is a `house-shaper` session.** Not a quick exception, not "while I'm here."
...
`house-orchestrator` §8, because the orchestrator is who runs the sweeps.
