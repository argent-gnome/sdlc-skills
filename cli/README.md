...
in later slices. The v2 skills that drive this CLI (the canonical `house-*` trio) and doctrine v2 ship in the
same slice as these commands but in a later unit.
...
- **Already-shaped work** — an existing spec/plan that predates the kernel — is adopted by a
  **`house-shaper` session**, not by a command: the shaper imports the artifacts into the slice's records
