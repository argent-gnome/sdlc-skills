...
> **Status (2026-07-29): all four are BUILT in slice `[0003]`** (S3a in the table above) — bound as
...
> all five with evidence (72 tests, `house validate` exit 0); the slice is not shipped until its merge
> gate and `live_check` pass. The list stays here as the durable record of what was found and where it
> went; it is not deleted when `[0003]` ships.
...
- **The R-1 fence-stripper is defeated by a fence quoted inside a fence.** `--strict` strips fenced
  ...
  kill, one level deeper. Sequenced after **S3a** ships.
