...
| Fix | **`validate --strict` nested-fence false positive** — the `[0003]`-build backlog item, minted. Replaces the byte-oriented `` ```…``` `` regex pair in `cli/lib/validate.js` with **line-oriented fence tracking**, so a three-backtick run appearing mid-line inside a string literal stops being read as a fence at all; reorders stripping to fences → HTML comments → code spans; picks up `~~~` fences, which the current stripper never handled. Slice `[0007]` — spec: [`slices/0007-validate-strict-nested-fence-false-positive/spec.md`](slices/0007-validate-strict-nested-fence-false-positive/spec.md); plan: [`slices/0007-validate-strict-nested-fence-false-positive/plan.md`](slices/0007-validate-strict-nested-fence-false-positive/plan.md) | **Shipped 2026-07-30** — [PR #13](https://github.com/argent-gnome/sdlc-skills/pull/13) merged to `main` as squash `85731f3`; `main` is at `2245f09` and pushed. All four gates on record: `spec_review` **approved** (jed), `plan_check` **GO_WITH_FIXES** (Fable), `merge_gate` **GO** (`fable-reviewer`), `live_check` **approved** — the user ran `house validate --strict` himself from a bare terminal on merged `main`, no output, exit 0, which is the [ADR-0003](adr/0003-no-hosted-ci-local-verification.md) bar this slice exists to protect, confirmed by a human rather than inherited from the reviewer's run. **The repo's last `--strict` red is gone:** on merged `main`, 77/77 tests, `house validate` exit 0, `house validate --strict` exit 0, with `docs/slices/0003-…/plan.md` **byte-identical** (blob `178a1e9`) — the residual carried since the `[0003]` build is closed as a *consequence* of the fix, not by editing the document. Hygiene complete: slice branch deleted local and remote, stale tracking ref pruned, one worktree, no stashes, tree clean. **One `work.discovered` at the ship, and it is the biggest thing this slice produced that was not the fix** — the merge landed a **strict superset** of the reviewed diff, because `base_sha` pointed at a local-only commit; benign here, verified so, and a hole in the merge gate in general. Routed to [the `[0007]` ship backlog](#backlog--discovered-in-the-0007-ship-2026-07-30) with its *why* recorded as [ADR-0005](adr/0005-reviewed-diff-equals-merged-diff.md) (`proposed`). Patch tier, appetite **0.5 session** held, one unit, 3 tasks, **3/3**, unit 01 **DONE** with zero code deviations. Suite **72 → 77**, `house validate` and `--strict` both exit 0 repo-wide. Merge gate **GO** (`fable-reviewer`, four lenses, `gates/merge_gate.yaml`) with **no R-1…R-5 violation** and zero findings — the reviewer re-ran everything itself: 77/0, R-3 verified by **blob hash** (`0003/plan.md` is `178a1e9` at base `c2f88db`, at HEAD, and in the working tree — byte-identical, not merely diff-clean), spec **R-5** re-measured independently by importing the shipped `stripFences()` against the old pipeline inlined (14-artifact scan set: exactly **1** verdict change, `0003/plan.md` RED→GREEN; **zero** GREEN→RED, same result across all 79 tracked `.md` files), the R-4 tamper check reproduced, and **10** adversarial marker-hiding probes clean. Both rulings that bound the build held: `0003/plan.md` passed **unmodified**, and R-5's re-measurement was the reviewer's, not inherited from the digest. Two preference-level reviewer notes, both resolved at this reconcile and neither a finding: the plan's post-approval edits are now disclosed in [its As-built](slices/0007-validate-strict-nested-fence-false-positive/plan.md) (including the wrong `house event --note` flag, corrected in place rather than left exiting 1), and the phantom-verb attribution is corrected below. Three `work.discovered` findings from the build → [the `[0007]` build backlog](#backlog--discovered-in-the-0007-build-2026-07-29), where they now sit alongside the ship's `base_sha` finding as **one coherent kernel-integrity slice candidate**; retro (optional at patch tier, written for the manual intervention and **extended with the ship-time facts**): [`slices/0007-…/retro.md`](slices/0007-validate-strict-nested-fence-false-positive/retro.md). Shaped 2026-07-29: spec and plan **approved**; `plan_check` **GO_WITH_FIXES** (Fable) with both must-fixes and all four advisories folded, none declined; kickoff brief **v1** in `slice.yaml`. Research digest: [`slices/0007-…/research/fence-stripper.md`](slices/0007-validate-strict-nested-fence-false-positive/research/fence-stripper.md). **Its findings are now two minted slices, not one candidate** — `[0008]` (the six writer/surface defects) and `[0009]` (`base_sha` reachability); see the Kernel and Guard rows below, and the split note at the head of [the `[0007]` build backlog](#backlog--discovered-in-the-0007-build-2026-07-29) |
| Kernel | **kernel record integrity — writers that misrepresent their records** — the `[0007]` build-and-ship backlog, minted and shaped. **Six** defects sharing one failure direction: each writer produces a record that *reads plausible and is wrong*. One `findSection(text, heading)` **locator** (last exact whole-line match; returns positions, performs no replacement) replaces the pattern-matching in both unit-report writers, which is the root-cause fix for three of the six at once; a `house adr <slice> attach <adr-id>` writer gives `slice.yaml`'s `adrs:` its one writer and regularizes `[0001]`'s hand-populated value *through the writer*; the unknown-flag guard names the accepted flags; `unit` validates its action before the unit lookup. Slice `[0008]` — spec: [`slices/0008-kernel-record-integrity-writers-that-misrepresen/spec.md`](slices/0008-kernel-record-integrity-writers-that-misrepresen/spec.md); plan: [`slices/0008-…/plan.md`](slices/0008-kernel-record-integrity-writers-that-misrepresen/plan.md); research: [`slices/0008-…/research/record-writer-anatomy.md`](slices/0008-kernel-record-integrity-writers-that-misrepresen/research/record-writer-anatomy.md) | **`ready` 2026-07-30 — shaped and handed off; NOTHING is dispatched.** Rigor **slice**, appetite **1 session**, one unit `01`, four tasks T1→T4, kickoff brief **v1** in `slice.yaml`. No branch is cut and **`base_sha` is `null`**. Gates on record: `spec_review` **approved**, then **R-2 revised after plan-check and re-approved** (`gates/spec_review.yaml`) — the first wording prescribed a *mechanism* (locate Heartbeats by its own heading, independent of any other heading) which forced an end-scan any note body can truncate, i.e. it mandated the very bug R-2 exists to prevent; the revision states the outcome and the design got **simpler** (one occurrence rule, both writers, no `which` parameter). Recorded as `deviation.raised` and in the spec's own revision block. `plan_check` **GO on round 2** after **GO_WITH_FIXES on round 1** (`gates/plan_check.yaml`, Fable, five lenses; round 1's three must-fixes were all red-test-evidence defects, verified folded; round 2 machine-simulated the plan's `findSection` and both writers rather than reading them, and found **no unnamed reachable case**). **The next session is a `house-orchestrator` session on this slice — do not re-shape it and do not re-run a gate.** **Sequencing: `[0008]` lands before `[0009]`**, because its `adrs:` writer is what lets `[0009]` register ADR-0005 on its own manifest through a writer instead of by hand |
| Guard | **`base_sha` reachability — the reviewed diff must equal the merged diff** — was **item (e)** of the kernel-integrity candidate and **split out into its own slice** at the `[0008]` shaping, because it is the only one of the seven that can let unreviewed content reach `main`. Two halves: a kernel guard (`house pr <id> --base-sha <sha>` verifies the sha is reachable from the remote default branch, degrading gracefully where the question is unanswerable) and an orchestrator ordering rule (**push `main` before cutting the slice branch**; commit every `house` record write before any `gh pr merge` or `git reset`), the second of which **subsumes the `[0004]` retro follow-up**. Design authority: **[ADR-0005](adr/0005-reviewed-diff-equals-merged-diff.md)**. Slice `[0009]` — research digest: [`slices/0009-base-sha-reachability-the-reviewed-diff-must-equ/research/base-sha-enforcement.md`](slices/0009-base-sha-reachability-the-reviewed-diff-must-equ/research/base-sha-enforcement.md) | **`shaping` 2026-07-30 — minted, researched, NO SPEC YET** (`spec.md` is the empty scaffold; `artifacts: {}` on the manifest). Rigor **slice**, appetite **1 session**. It needs a **`house-shaper`** session, and it is **sequenced behind `[0008]`**. **ADR-0005 is `proposed` and its `adr_review` gate has deliberately NOT run** — `[0009]`'s spec is what must rule on the **enforcement strength** ADR-0005 left open (refusing is the fail-closed instinct, but `house pr` is a recording command and a refusal there can strand a slice with the PR already open), and on where the ordering rule lands: the `house-orchestrator` skill, doctrine §5, or both. Approving the ADR before that ruling exists would settle by signature what the spec is supposed to settle by argument |
...
**SPLIT AND MINTED 2026-07-30 — this framing is now TWO slices, not one candidate.** It was written as a
single candidate so that a shaper would not mint the parts separately; the `[0008]` shaping pass read it,
agreed with the theme, and split it on the one axis that matters — **`[0008]` owns every defect where a
writer or the surface documenting it misrepresents the record it is producing; `[0009]` owns the one defect
that can let unreviewed content reach `main`.** Two items below were never in the original table: **(f)** was
raised as its own `work.discovered` at the `[0007]` ship reconcile (`2026-07-30T01:59:04Z`) and lived only in
`.house/events.jsonl` until now, and **(g)** was found while specifying (a) and (b). **Seven items in total —
six of them `[0008]`'s.**

**`[0008]` — the six writer/surface defects.** See the **Kernel** row in the program table above. All six fixes
land in `cli/lib/slices.js`, plus one guard message in `cli/bin/house.js` and one dispatch-wording fix. The
first three are **one root cause** — `.replace` used as a document-editing primitive — retired by a single
`findSection(text, heading)` **locator** that returns positions and performs no replacement.

| # | Defect | Fix | Where | Spec rule |
|---|---|---|---|---|
| a | `house unit … finalize` truncates the report at an **unanchored** `/## Result[\s\S]*$/` — first match anywhere in the body | **last exact whole-line match** — *not* the `/^## Result[\s\S]*$/m` anchor this row originally prescribed, which still destroys a line-start quotation of the heading and a heading inside a fence | `cli/lib/slices.js:244` | R-1 |
| b | the sibling `heartbeat` branch does a **first-occurrence plain-string** `cur.replace('\n## Result', …)` — inserts at the wrong place, exit 0, nothing deleted | one occurrence rule shared by both writers through the locator: `finalize` replaces from the last match, `heartbeat` inserts before it | `cli/lib/slices.js:235` | R-2 (revised) |
| g | **`$`-pattern injection** — both writers pass a **template literal as the replacement argument** to `.replace`, so `$&`, `` $` ``, `$'` and `$1` inside a `--note` are interpreted as replacement patterns rather than inserted as text | a locator that returns positions has no replacement-pattern position to inject through — the same fix as (a) and (b), which is why all three are one root cause | `cli/lib/slices.js:235`, `:244` | R-3 |
| c | `house event` takes `--payload`, not `--note`; the wrong spelling exits 1 under the `[0003]` R-5 flag guard, and the guard never says what the right spelling is | the guard message names the command's accepted flags — **not** a flag rename, which would break every skill file and doctrine | `cli/bin/house.js` flag guard | R-6 |
| d | `house unit <slice> report <unit>` **does not exist** (`finalize` emits `unit.report`), and the action is validated *after* the unit lookup, so the typo reports `no such unit: undefined` — naming the wrong argument entirely | hoist the action check above the unit lookup — **not** a `report` alias, which would be a second name for one writer | `cli/lib/slices.js:211` / dispatch prompt | R-7 |
| f | `slice.yaml`'s **`adrs:` field has no writer and no kernel reader** — `adrs: []` in the mint literal is its only appearance anywhere in the kernel, and `--adr` on `house new` selects the ADR-*minting* branch and returns early, so it never reaches a slice manifest | `house adr <slice> attach <adr-id>` — give the field its one writer, refuse an ADR with no file under `docs/adr/`, and re-record `[0001]`'s existing value *through* the writer | `cli/lib/slices.js:76`; `docs/slices/0001-…/slice.yaml:13-14` | R-5 |

**(f) is a live doctrine §1 breach, not merely a dead field — which is why `[0008]` gives it a writer instead
of deleting it.** `docs/slices/0001-house-v2-s2-skills-rewrite/slice.yaml:13-14` carries a **hand-populated**
value, and the ADR it names — [ADR-0004](adr/0004-house2-coexistence-and-advisory-hooks.md) — **exists**. The
content is true; only the provenance is wrong. A CLI-owned field written by hand has been sitting on `main`
since S2, invisible to `house validate` because there is no manifest key allowlist (deferred — see the
`[0008]`-shaping backlog below). Deleting the field would discard a correct fact and require hand-editing
every CLI-owned manifest that declares it (the spec counted eight; `[0009]`'s mint made it nine); re-recording the same value *through* the new writer closes the breach
without a single hand-edit, which is why R-5 is a writer and not a deletion.

**(e) — `base_sha` reachability — is slice `[0009]` now, not an item on this list.** It was the only one of the
seven that can let unreviewed content reach `main`, and that is what earned it its own slice at **slice**
rigor rather than a row in a patch-tier table. **Do not build it from here:** its design authority is
**[ADR-0005](adr/0005-reviewed-diff-equals-merged-diff.md)**, and the enforcement-strength question the ADR
deliberately left open is `[0009]`'s spec to rule on. See the **Guard** row in the program table above and
[the `[0007]` ship backlog](#backlog--discovered-in-the-0007-ship-2026-07-30) below.

**What unified all seven is the failure *direction*, and it still does:** each one produces a record that
reads plausible and is wrong, which is the exact class the kernel's one-writer-per-field design exists to
prevent. What *split* them is blast radius. **Only three of the seven were found by the build that named this
backlog** — (b) came from the `[0007]` merge-gate reviewer, (e) from the ship, (f) from the ship's reconcile,
and (g) from writing `[0008]`'s spec — which is the argument for having taken them together while the context
was fresh. **(a), (b) and (g) additionally retire a rule nobody should have to know** — *a unit report must
not contain the result heading's literal text anywhere in its body* — and it is **last-match, not anchoring,
that actually retires it.**
...
completed — and it is **item (e)** of the kernel-integrity framing at the head of
[the `[0007]` build backlog](#backlog--discovered-in-the-0007-build-2026-07-29) above, **split out and minted
2026-07-30 as its own slice `[0009]`** when that framing became two slices (the **Guard** row in the program
table). Read the ADR for the invariant; read this bullet for what to build.

- **`base_sha` can point at a local-only commit, so the reviewed diff can be narrower than the merged diff —
  a hole in the merge gate.** **Minted 2026-07-30 as slice `[0009]`** (`shaping`, research digest written, no
  spec yet), sequenced behind `[0008]`. **What happened:** at branch time the orchestrator
...
### Backlog — deferred at the `[0008]` shaping (2026-07-30)

Two rabbit holes `[0008]`'s spec names and **deliberately does not take** (`spec.md` → Rabbit Holes). Both are
real, both sit in the same neighbourhood as the fix, and both are recorded here so that the deferral is a
decision on record rather than an omission nobody notices.

- **A manifest key allowlist for `house validate`.** The check that would have caught `[0001]`'s
  hand-populated `adrs:` — a CLI-owned field written by hand, on `main` since S2, visible to nothing.
  Tempting to take while fixing R-5, and refused because **it is a different defect class: an absent check,
  not a lying writer.** `[0008]` fixes writers; an allowlist asks a validator question — *does this manifest
  contain a key the kernel does not own* — with its own scope to settle (what the key set is, whether an
  unknown key is an error or a warning, what happens to a manifest written by an older kernel). Unminted,
  unblocked, patch-tier candidate; a natural rider on whichever slice next opens `cli/lib/validate.js`.
  **Note the ordering:** `[0008]` closes the one instance, this closes the class — until it lands, the *next*
  hand-edit of a `slice.yaml` is exactly as invisible as `[0001]`'s was.
- **Rewriting the unit-report format to use machine markers**, the way `renderDevState` already does for
  `docs/dev-state.md`. **Structurally immune to this entire class:** a marker-delimited region cannot be
  confused by prose that happens to quote a heading, so R-1, R-2 and the retired *"no literal heading text in
  the body"* rule all stop being questions instead of being answered. Refused on **size, not on merit** — it
  is a format migration for **nine existing shipped unit reports** plus the template and both writers, and it
  is not a 1-session change. Unminted, unblocked. **If it is ever taken, `[0008]`'s locator is the thing it
  deletes** — take it as a replacement, never as a layer on top of the locator.
