...
The wait is over; the board's top local item is finishing `[0004]` (see its status cell in the table).
...
- **The R-1 fence-stripper is defeated by a fence quoted inside a fence.** `--strict` strips fenced
  ...
  needs its own patch-tier slice when someone wants it. It is the only `--strict` red in the repo, and —
  alongside the `docs/index.html` item in [the `[0004]` build backlog](#backlog--discovered-in-the-0004-build-2026-07-29)
  below — one of two unminted, unblocked items on this board.

### Backlog — discovered in the `[0004]` build (2026-07-29)

- **`docs/index.html` is a published live surface with a false claim, and nobody owns it.** The page
  still describes the repo as a "plugin-free **pair** of Claude Code skills" (lede), refers to "these
  two skills" in the body, and its footer reads *"two skills: `house-orchestrator` +
  `house-builder`"*. Three skills ship, on canonical names, as of the 2026-07-29 cutover. Two slices
  passed the page by for defensible reasons and left a hole between them: `[0005]` **declined** the
  quickstart/process-v2 cards at plan-check because `index.html` is v1 surface it did not own, on the
  expectation the cutover would take it; `[0004]` then scoped itself to the rename, the archive, and
  the live surfaces its spec named — `docs/index.html` is not among them, and its No-Gos bar a v2
  rewrite of the v1 pages. Net owed: correct the two-skills claim in all three places, and decide
  whether the page gets quickstart/process-v2 cards. Small — a patch-tier slice, or a rider on
  whichever slice next opens that page. Surfaced in the `[0004]` plan's As-built ("Outstanding") and
  as a `work.discovered` event, so the merge gate sees it rather than inherits it.
